...
| `npm run typecheck` / `npm run lint` appear to never exit (no diagnostics for minutes); even `git diff --stat` can stall | NOT a tooling or Node-version bug. This 8 GB machine near 0 free pages (Codex+Claude+Chrome resident) makes cold tsc/eslint sit in swap/IO wait: measured tsc 185s wall vs 7s CPU cold, then 3s warm; a backgrounded eslint without a TTY got App Nap–throttled to ~61 min wall vs 7s CPU. Both exit 0 given time. | Diagnose with `caffeinate -i /usr/bin/time -p <cmd> > log 2>&1` and allow ≥5 min cold. Keep warm artifacts: `tsconfig.tsbuildinfo` (incremental tsc ≈2s) and `.eslintcache` (`lint` script passes `--cache`; cached run ≈1.5s). Never delete these caches to "clean up", never run gates in parallel, and prefix long gates with `caffeinate -i`. |
