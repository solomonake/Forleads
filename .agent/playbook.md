...
| CodeQL `js/weak-cryptographic-algorithm` (high) on a v5-UUID helper after wiring it to user data | RFC-4122 **v5 UUIDs use SHA-1**. CodeQL's taint rule fires when *sensitive* input (a Google `sub`, an email) flows into SHA-1 — even though the SHA-1 is just deriving an id, not protecting anything. On `main` the same helper was fine because it only saw internal slugs. | Split the derivation. Keep `uuidV5()` (SHA-1) **only** for already-persisted internal slugs (changing it orphans rows). For user-derived ids use `deterministicUuid()` (SHA-256, version-8) in `core/ids.ts`. No sensitive data reaches SHA-1 → alert clears, slug ids unchanged. Don't dismiss the alert; the split is the real fix. |
